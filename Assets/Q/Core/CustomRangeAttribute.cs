...
public class CustomRangeDrawer: PropertyDrawer {
  public override void OnGUI (Rect position, SerializedProperty property, GUIContent label) {
    ...
    float currentValue = property.propertyType == SerializedPropertyType.Float? property.floatValue
        : property.intValue;
    float newValue = EditorGUI.Slider (position, label, currentValue,
    customRangeAttribute.Options[0],
    customRangeAttribute.Options[customRangeAttribute.Options.Length - 1]);
    ...
    if (EditorGUI.EndChangeCheck ()) {
      ...
      if (index < 0) {
        ...
        float rightValue
            = customRangeAttribute.Options[
              Mathf.Min (index, customRangeAttribute.Options.Length - 1)];
