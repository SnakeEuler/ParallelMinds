...
public class InformAttribute: PropertyAttribute {
  ...
  public InformAttribute (string message) { this.message = message; }
}
...
public class InformDrawer: PropertyDrawer {
  public override void OnGUI (Rect position, SerializedProperty property, GUIContent label) {
    ...
    Rect propertyRect = new Rect (position.x, position.y, propertyWidth,
    ...
    if (availableSpace >= messageSize.x) {
      ...
      Rect infoBoxRect = new Rect (position.x + propertyWidth + 4f, position.y, availableSpace,
      EditorGUIUtility.singleLineHeight);
